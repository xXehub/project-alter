...
module.exports = client => {
    client.on("interactionCreate", async (interaction) => {
        ...
        if (!member.voice.channel)
            return interaction?.reply({
                content: `<:no:939372664559132723> **Please join a Voice Channel first!**`,
                ephemeral: true
            })      
            
        var player = client.manager.players.get(interaction?.guild.id);
        if (interaction?.customId != "Join" && interaction?.customId != "Leave" && (!player || !player.queue || !player.queue.current))
            return interaction?.reply({content: "<:no:939372664559132723> Nothing Playing yet", ephemeral: true})
        ...
        if (player && member.voice.channel.id !== player.voiceChannel)
            return interaction?.reply({
                content: `<:no:939372664559132723> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`,
                ephemeral: true
            })
        ...
        if(player && interaction?.customId != `Join` && interaction?.customId != `Lyrics` && check_if_dj(client, member, player.queue.current)) {
                return interaction?.reply({embeds: [new MessageEmbed()
                  ...
                  .setTitle(`<:no:939372664559132723> **You are not a DJ and not the Song Requester!**`)
        ...
        switch(interaction?.customId){
            ...
            case "Autoplay": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.get(`autoplay`) ? `<a:Check_List_Ijo:870279724906197033> **Enabled Autoplay**`: `<:no:939372664559132723> **Disabled Autoplay**`}`)
                  .setFooter(client.getFooter(`👥 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
            ...
            case "Song": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.trackRepeat ? `<a:Check_List_Ijo:870279724906197033> **Enabled Song Loop**`: `<:no:939372664559132723> **Disabled Song Loop**`}`)
                  .setFooter(client.getFooter(`👥 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
            ...
            case "Queue": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.queueRepeat ? `<a:Check_List_Ijo:870279724906197033> **Enabled Queue Loop**`: `<:no:939372664559132723> **Disabled Queue Loop**`}`)
                  .setFooter(client.getFooter(`👥 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
    ...
    client.on("messageCreate", async message => {
        ...
        if (prefixRegex.test(message.content)) {
            ...
            if (cmd || cmd.length === 0) return// message.reply("<:no:939372664559132723> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
            ...
            {
                return// message.reply("<:no:939372664559132723> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
            }
        }
        ...
        if (!channel) return message.reply("<:no:939372664559132723> **Please join a Voice Channel first!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 5000)})
        ...
        if (player && channel.id !== player.voiceChannel) return message.reply(`<:no:939372664559132723> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`).then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
...
function generateQueueEmbed(client, guildId, leave){
    ...
    var embeds = [
      ...
      new MessageEmbed()
        ...
        .setImage(guild.banner ? guild.bannerURL({size: 4096}) : "https://cdn.discordapp.com/attachments/755272082387894323/938356786413592626/standard_17.gif")
    ...
    if(!leave && player && player.queue && player.queue.current){
        embeds[1].setImage(`https://img.youtube.com/vi/${player.queue.current.identifier}/mqdefault.jpg`)
            ...
            .setAuthor(`${player.queue.current.title}`, "https://cdn.discordapp.com/attachments/755272082387894323/938356786413592626/standard_17.gif", player.queue.current.uri)
    ...
    var joinbutton = new MessageButton().setStyle('SUCCESS').setCustomId('Join').setEmoji(`938937482714701865`).setLabel(`Join`).setDisabled(false);
    var leavebutton = new MessageButton().setStyle('DANGER').setCustomId('Leave').setEmoji(`938937460140953630`).setLabel(`Leave`).setDisabled();
